...
class VK:
    ...
    def vk_fotos(self):
        ...
        params = {'owner_id': self.id,'access_token': self.token,'rev':0,'count':5,'extended':1,'album_id':'profile','photo_sizes': True,'v': self.version}
        response = requests.get(url, params=params)
        for el in response.json()['response']['items']:
            for i in el['sizes']:
                if i['type'] == 'm' and el['likes']['count']!=0:
                   url_image = i['url']
                   ...
                   with open(f'image//{file_name}', 'wb') as f:
                        ...
                        time.sleep(0.5)
                elif i['type'] == 'z' and el['likes']['count']==0:
                    n+=1
                    url_image = i['url']
                    file_name = f'{el['likes']['count']}_{datetime.now().strftime("%d_%m_%Y_%H_%M_%S")}.jpg'
                    response = requests.get(url_image)
                    with open(f'Image//{file_name}', 'wb') as f:
                        f.write(response.content)
                        time.sleep(0.5)
